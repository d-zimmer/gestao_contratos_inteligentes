...
from contratos_inteligentes.models import RentalContract, ContractEvent, User, Payment
...
class ContractAPITestCase(TestCase):
    def setUp(self):
        ...
        self.payment_url = lambda contract_id: reverse('register_payment', args=[contract_id])
        self.terminate_url = lambda contract_id: reverse('terminate_contract', args=[contract_id])
    ...
    def test_create_contract_invalid_dates(self):
        ...
        invalid_data['private_key'] = self.contract_data['private_key_landlord']  # Incluir a chave privada
    ...
    def test_create_contract_web3_connection_error(self, mock_web3):
        ...
        self.assertIn("Falha na conexão com a rede Ethereum", response.json().get('error', ""))
    ...
    def test_sign_contract_already_signed(self):
        ...
        if third_sign_response.status_code == 500:
            print(f"Erro ao tentar assinar o contrato novamente: {third_sign_response.json()}")
    ...
    def test_sign_contract_not_found(self):
        ...
        response = self.client.post(self.sign_url(999), data={
            'private_key': self.contract_data['private_key_landlord'],
            'user_type': 'landlord'
        }, format='json')
    ...
    def test_register_payment(self):
        ...
        payment_data = {
            'private_key': self.contract_data['private_key_tenant'],  # A chave privada do inquilino
    ...
    def test_terminate_contract_by_unauthorized_user(self):
        ...
        response = self.client.post(f'/api/contracts/{contract_id}/terminate/', {
    ...
    def test_simular_tempo(self):
        ...
        response = self.client.post(f'/api/contracts/{contract_id}/simular_tempo/', data={
            'simulated_date': simulated_date,
            'private_key': self.contract_data['private_key_landlord']
        }, format='json')

        self.assertEqual(response.status_code, 200, f"Erro ao simular tempo: {response.json()}")
...
class UtilsTests(TestCase):
    ...
    def test_check_connection_failure(self, mock_web3):
        ...
        self.assertEqual(str(context.exception), "Não conectado à rede Ethereum. Verifique sua conexão.")
    ...
    def test_log_contract_event_missing_data(self):
        ...
        with self.assertRaises(ValueError) as context:
            log_contract_event(None, "create", "0x1234567890abcdef1234567890abcdef12345678")
    ...
    def test_log_contract_event_with_none_contract(self):
        ...
        with self.assertRaises(ValueError) as context:
            log_contract_event(None, 'create', '0xabcdef1234567890abcdef1234567890abcdef12')
        self.assertEqual(str(context.exception), "O contrato não pode ser None.")
    ...
    def test_log_contract_event_with_empty_event_type(self):
        ...
        with self.assertRaises(ValueError) as context:
            log_contract_event(self.contract, '', '0xabcdef1234567890abcdef1234567890abcdef12')
    ...
    def test_log_contract_event_with_empty_user_address(self):
        ...
        self.assertEqual(str(context.exception), "O endereço do usuário não pode ser vazio.")
...
class UserModelTest(TestCase):
    def test_invalid_wallet_address_length(self):
        user = User(name="John Doe", email="john@example.com", wallet_address="0x12345", is_landlord=True)
        with self.assertRaises(ValidationError):
            user.full_clean()
